	...
	public:
		...
		class Not_Expressing_RET_Ligand : public robochart::State 
		{
			public:
				...
				void Entry()
				{
					S_LTo_State_Machine->expressing_RET = false;
				}
			private:
				std::shared_ptr<LTo_Step> R_LTo_Step;
				std::shared_ptr<LTo_Controller> C_LTo_Controller;
		...
		class Expressing_RET_Ligand : public robochart::State 
		{
			public:
				...
				void Entry()
				{
					S_LTo_State_Machine->expressing_RET = true;
					R_LTo_Step->new_LTin_binding = detect_cell_bind(R_LTo_Step->LTo_loc, std::string("LTin"));
				}
			private:
				std::shared_ptr<LTo_Step> R_LTo_Step;
				std::shared_ptr<LTo_Controller> C_LTo_Controller;
		...
		class Upregulate_Adhesion_Molecules : public robochart::State 
		{
			public:
				...
				{
					R_LTo_Step->new_LTin_binding = detect_cell_bind(R_LTo_Step->LTo_loc, std::string("LTin"));
					R_LTo_Step->new_LTi_binding = detect_cell_bind(R_LTo_Step->LTo_loc, std::string("LTi"));
		...
		class Expressing_Chemokines : public robochart::State 
		{
			public:
				...
				void Entry()
				{
					if (R_LTo_Step->new_LTin_binding == true) 
					{
						if (R_LTo_Step->adhesionExpressed < R_LTo_Step->maxVCAMProbability)
						{
							R_LTo_Step->adhesionExpressed = R_LTo_Step->adhesionExpressed + R_LTo_Step->adhesionIncrement;
					...
					{
						if (R_LTo_Step->chemokineExpressed < R_LTo_Step->maxChemokineExpression)
						{
							R_LTo_Step->chemokineExpressed = R_LTo_Step->chemokineExpressed + R_LTo_Step->chemokineIncrement;
						}
					}
					if (R_LTo_Step->adhesionExpressed >= R_LTo_Step->maxVCAMProbability && R_LTo_Step->chemokineExpressed >= R_LTo_Step->maxChemokineExpression)
					{
